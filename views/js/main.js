...
var resizePizzas = function(size) { 
  ...
  //CHANGE: number of random pizzas is 100, REMOVED .length to avoid reference-- do not need to iterate just do for all
  function changePizzaSizes(size) {
    //CHANGE: since all pizzas are the same size depending on the slider and therefore there is no need to check every pizza's size: I took dx and new width out of the loop
...
//MOVED frame var into updatePositions()
...
function updatePositions() {
 ...
 //MOVED here to be a local variable
  var frame = 0;
  frame++;
  window.performance.mark("mark_start_frame");

  var items = document.querySelectorAll('.mover');
  //CHANGE TO i<50 as we know there are 50 movers
  for (var i = 0; i < 30; i++) {
...
//MOVED height, width and src to css
